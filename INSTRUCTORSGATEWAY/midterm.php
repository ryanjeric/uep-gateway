   ...
   print "<form method='POST'>
    ...
    if(mysqli_num_rows($sel_students) > 0) {
        ...
        echo "<td>$num<input type='hidden' name='student_id[]' value='$sid'></td>";
            ...
            $sel_studentgrade = mysqli_query("SELECT * FROM midtermgrade WHERE term = $_SESSION[sem] AND courseid = $course_ID AND studentid = $sid LIMIT 1");
...
if(isset($_POST['save'])) {
   ...
   foreach($_POST['student_id'] as $sid) {
        ...
        $check = mysqli_query("SELECT * FROM midtermgrade WHERE term = $_SESSION[sem] AND courseid = $course_ID AND studentid = $sid");
