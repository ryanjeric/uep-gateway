...
<div d2>
    <span class='divhead'>
    <a class='spanhead'>Prefinal Grade</a>
    </span>
<br><br>
<?php
$TERM = $_GET['term'];
...
$SQL = mysqli_query("SELECT * FROM grading_overscore WHERE term = $TERM AND courseid = $course_ID and grading = '$grading' ") or die(mysqli_error());
...
echo "<center><br>";
$sel_subject = mysqli_query("SELECT * FROM schedule WHERE courseid = $course_ID AND term = $TERM AND classtype = 'LAB' ") or die(mysqli_error());
   ...
   print "<form method='POST'>
    ...
    if(mysqli_num_rows($sel_students) > 0) {
        ...
        echo "<td>$num<input type='hidden' name='student_id[]' value='$sid'></td>";
            ...
            $sel_studentgrade = mysqli_query("SELECT * FROM prefinalgrade WHERE term = $TERM AND courseid = $course_ID AND studentid = $sid LIMIT 1");
...
<button onClick='parent.location=\"../registrarshome.php\"' style='float:left;'>HOME</button>
        <button onClick='parent.location=\"index.php\"' style='float:left;'>REPORTS</button>
                ...
                ";
?><button onclick="goBack()">Go Back</button></span>
<br><br>
</div>
</div>
</body>
