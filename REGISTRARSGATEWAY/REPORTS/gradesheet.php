...
<html>
<head>
<title>Registrar's Gateway</title><script>
function goBack() {
    window.history.back();
}
</script>
...
<tr>
    ...
    <td>$course_units</td>
    <td>";
$sched=mysqli_query("SELECT * FROM schedule where term=$TERM and courseid=$course_ID ") or die(mysqli_error());
...
$count = 1;
  $sel_students = mysqli_query("SELECT * FROM studentssubjtbl LEFT JOIN studentstbl ON studentssubjtbl.studentid = studentstbl.idno WHERE term=$TERM AND courseid = $course_ID ORDER BY lname") or die(mysqli_error());
   if(mysqli_num_rows($sel_students) > 0) {
    ...
    $num = $count++;
    $sid = $stud['idno'];
    $sel_prelimgrade = mysqli_query("SELECT * FROM prelimgrade WHERE term=$TERM AND courseid= $course_ID AND studentid = $sid");
        $prelim = mysqli_fetch_array($sel_prelimgrade);
            $prelim_grade = $prelim['grade_prelim'];
            $COMPLY = $prelim['complied'];
    $sel_midtermgrade = mysqli_query("SELECT * FROM midtermgrade WHERE term=$TERM AND courseid= $course_ID AND studentid = $sid");
        $midterm_grade = mysqli_fetch_array($sel_midtermgrade)['grade_midterm'];
    $sel_prefinalgrade = mysqli_query("SELECT * FROM prefinalgrade WHERE term=$TERM AND courseid= $course_ID AND studentid = $sid");
        $prefinal_grade = mysqli_fetch_array($sel_prefinalgrade)['grade_prefinal'];
    $sel_finalgrade = mysqli_query("SELECT * FROM finalgrade WHERE term=$TERM AND courseid= $course_ID AND studentid = $sid");
    ...
    if($FINAL_GRADE > 75) { $remarks = 'Passed';}
     ...
     if($COMPLY == 'Yes') {
       $sel_gradesheet = mysqli_query("SELECT * FROM gradesheet WHERE term=$TERM AND courseid= $course_ID AND studentid = $sid ");
...
<button onClick='parent.location=\"../registrarshome.php\"' style='float:left;'>HOME</button>
        <button onClick='parent.location=\"index.php\"' style='float:left;'>REPORTS</button>

                ";
?><button onclick="goBack()" style='float:left;'>Go Back</button></span>
