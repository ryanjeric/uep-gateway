...
<div d2>
    <span class='divhead'>
    <a class='spanhead'>Pre-Final Grades</a>
    <?php echo "<a href='grades.php?id=$_SESSION[grades_courseID_registrar]' class='back'>&#8629 Go Back</a>";?>
    </span>
<br><br>
<?php
if(!isset($_SESSION['grades_courseID_registrar'])) {
    header('Location:index.php');
}
$course_ID = $_SESSION['grades_courseID_registrar'];
...
$SQL = mysqli_query("SELECT * FROM grading_overscore WHERE term = $sem AND courseid = $course_ID and grading = '$grading' ") or die(mysqli_error());
...
echo "<center><br><button type='submit' class='unlock'>UNLOCK</button>";
$sel_subject = mysqli_query("SELECT * FROM schedule WHERE courseid = $course_ID AND term = $sem AND classtype = 'LAB' ") or die(mysqli_error());
if(mysqli_num_rows($sel_subject) > 0) {
   ...
   print "<form method='POST'>
    ...
    <tr><td rowspan='5'><b>#</b></td>
        ...
        <td rowspan='5'><b>Prefinal Grade</b></td>
    </tr>
    <tr>
    ...
    $sel_students = mysqli_query("SELECT * FROM studentssubjtbl LEFT JOIN studentstbl ON studentssubjtbl.studentid = studentstbl.idno WHERE term = $sem AND courseid = $course_ID ORDER BY lname") or die(mysqli_error());
    if(mysqli_num_rows($sel_students) > 0) {
        ...
        echo "<td>$num<input type='hidden' name='student_id[]' value='$sid'></td>";
            ...
            $sel_studentgrade = mysqli_query("SELECT * FROM prefinalgrade WHERE term = $sem AND courseid = $course_ID AND studentid = $sid LIMIT 1");
            if(mysqli_num_rows($sel_studentgrade) > 0) {  
                while($grade = mysqli_fetch_array($sel_studentgrade)) {
                    ...
                    $grade_prefinal = $grade['grade_prefinal'];
...
echo "</table><br>";
        ?><br><br>
       <?php echo" <a href='prefinal_print.php' target='_blank' buttonlike>Printable View</a>
    </form>";
}
...
if(isset($_POST['save'])) {
    ...
    $check = mysqli_query("SELECT * FROM grading_overscore WHERE term = $sem AND courseid = $course_ID AND grading ='$grading' ") or die(mysqli_error());
   ...
   foreach($_POST['student_id'] as $sid) {
        ...
        $check = mysqli_query("SELECT * FROM prefinalgrade WHERE term = $sem AND courseid = $course_ID AND studentid = $sid");
        $prefinalgrade_ID = mysqli_fetch_array($check)['prefinalgradeid'];
        if(mysqli_num_rows($check) > 0) {
            if ($withlab == 'T') {
             $up_grade = mysqli_query("UPDATE prefinalgrade SET
                    q1 = '$_POST[$Q1]',
                    q2 ='$_POST[$Q2]',
                    ...
                    grade_lab = '$GRADE_LAB',
                    grade_prefinal = '$GRADE'
                    WHERE prefinalgradeid = $prefinalgrade_ID")or die(mysqli_error());
            }
            else {
                $up_grade = mysqli_query("UPDATE prefinalgrade SET
                q1 = '$_POST[$Q1]',
            ...
            if ($withlab == 'T') {
            $ins_grade = mysqli_query("INSERT INTO prefinalgrade VALUES (
                    '',
                    '$sem',
                    '$course_ID',
                    '$sid',
...
$date_change = date('M d, Y');
echo "<div class='hidden'>
        <div class='hidden-form'>
        <form method='POST'>
            <table class='hidden-table'>
                <tr>
                    <td>OR #:</td><td><input type='text' name='OR' required></td>
                </tr>
                <tr>
                    <td>Reason:</td><td><input type='text' name='reason' required></td>
                </tr>
                <tr>
                    <td>Date Change:</td><td>$date_change</td>
                </tr>
                <tr>
                    <td></td>
                    <td><button type='submit' name='post'>POST</button> <button type='submit' name='cancel' id='cancel' formnovalidate>CANCEL</button></td>
                </tr>
            </table>    
        </form>    
        </div>
...
if(isset($_POST['post'])) {
    $OR = mysqli_real_escape_string(strtoupper($_POST['OR']));
    $Reason = mysqli_real_escape_string(ucwords(strtolower($_POST['reason'])));
    $DateChange = $date_change;

    $sql_post = mysqli_query("INSERT INTO changegrade VALUES ('','$OR','$Reason','$DateChange','$_SESSION[sem3]','$course_code')");

    $_SESSION['prefinal_change'] = 'ON';
    header('Location:'.$grading.'_change.php');
}
?>