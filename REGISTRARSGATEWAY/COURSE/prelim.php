...
<div d2>
    <span class='divhead'>
    <a class='spanhead'>Prelim Grades</a>
   ...
   print "<form method='POST'>
    ...
    if(mysqli_num_rows($sel_students) > 0) {
        ...
        echo "<td>$num<input type='hidden' name='student_id[]' value='$sid'></td>";
            ...
            $sel_studentgrade = mysqli_query("SELECT * FROM prelimgrade WHERE term = $sem AND courseid = $course_ID AND studentid = $sid LIMIT 1");
...
echo "</table><br>";
        ?><br><br>
       <?php echo" <a href='prelim_print.php' target='_blank' buttonlike>Printable View</a>
    </form>";
}
...
if(isset($_POST['save'])) {
   ...
   foreach($_POST['student_id'] as $sid) {
        ...
        $check = mysqli_query("SELECT * FROM prelimgrade WHERE term = $sem AND courseid = $course_ID AND studentid = $sid");
