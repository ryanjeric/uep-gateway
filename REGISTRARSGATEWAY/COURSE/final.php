...
<div d2>
    <span class='divhead'>
    <a class='spanhead'>Tentative Final Grades</a>
...
if(mysqli_num_rows($sel_subject) > 0) {
   ...
   print "<form method='POST'>
    ...
    <tr><td rowspan='5'><b>#</b></td>
        ...
        <td rowspan='5'><b>Final Grade</b></td>
    </tr>
    <tr>
    ...
    if(mysqli_num_rows($sel_students) > 0) {
        ...
        echo "<td>$num<input type='hidden' name='student_id[]' value='$sid'></td>";
            ...
            $sel_studentgrade = mysqli_query("SELECT * FROM finalgrade WHERE term = $sem AND courseid = $course_ID AND studentid = $sid LIMIT 1");
...
echo "</table><br>";
        ?><br><br>
       <?php echo" <a href='final_print.php' target='_blank' buttonlike>Printable View</a>
    </form>";
}
...
if(isset($_POST['save'])) {
   ...
   foreach($_POST['student_id'] as $sid) {
        ...
        $check = mysqli_query("SELECT * FROM finalgrade WHERE term = $sem AND courseid = $course_ID AND studentid = $sid");
        $finalgrade_ID = mysqli_fetch_array($check)['finalgradeid'];
        if(mysqli_num_rows($check) > 0) {
            if ($withlab == 'T') {
             $up_grade = mysqli_query("UPDATE finalgrade SET
                    q1 = '$_POST[$Q1]',
                    q2 ='$_POST[$Q2]',
                    ...
                    grade_lab = '$GRADE_LAB',
                    grade_final = '$GRADE'
                    WHERE finalgradeid = $finalgrade_ID")or die(mysqli_error());
            }
            else {
                $up_grade = mysqli_query("UPDATE finalgrade SET
                q1 = '$_POST[$Q1]',
            ...
            if ($withlab == 'T') {
            $ins_grade = mysqli_query("INSERT INTO finalgrade VALUES (
                    '',
                    '$sem',
                    '$course_ID',
                    '$sid',
